...
describe "Fundraiser Creation" do
  ...
  it "should create a new fundraiser and land on the edit page" do
    ...
    @browser.override_api_response_data :create_fundraiser, data: factory(:fundraiser)
  ...
  it "should see edit button on fundraiser show page" do
    login_with_email!
    @browser.override_api_response_data :get_fundraisers, data: [factory(:fundraiser)]
  ...
  describe "with newly created fundraiser" do
    let(:fundraiser) { OpenStruct.new(factory(:unpublished_fundraiser)) }
    before do
      login_with_email!
      @browser.override_api_response_data :get_fundraisers, data: [factory(:unpublished_fundraiser)]
      # goto fundraiser page
      @browser.goto '#account/fundraisers'
      @browser.link(text: fundraiser.title).wait_until_present
      link = @browser.links(text: fundraiser.title).last
      @browser.override_api_response_data :get_fundraiser, data: factory(:unpublished_fundraiser)
      @browser.goto link.attribute_value('href') # goto fundraiser show page
    ...
    it "should fill in description" do
      @browser.override_api_response_data :update_fundraiser, data: factory(:unpublished_fundraiser)
      ...
      description_element.set(
        ...
        "because I feel like it", :tab
      )
      fundraiser.description = description_element.value

      # goto preview and make sure description is there
      text = <<-description
        ## This is my awesome fundraiser
        Please give me money.
        ## FAQs
        > y u so cool?
        because I feel like it
      description
      @browser.override_api_response_data :get_fundraiser, data: factory(:unpublished_fundraiser).merge(description: text)
      @browser.div(id: 'fundraiser-card-preview').click
      @browser.p(text: factory(:unpublished_fundraiser)[:description]).wait_until_present
    end
    ...
    it "should fill in funding goal" do
      ...
      @browser.div(id: 'fundraiser-card-preview').wait_until_present

      # goto preview and make sure description is there
      @browser.div(id: 'fundraiser-card-preview').click

      @browser.text.should match(/You need to provide all of the necessary data to publish your fundraiser/)
    end
    ...
    it "should fill in duration" do
      ...
      @browser.span(text: /#{fundraiser.days_remaining}/).wait_until_present
    end
    ...
    describe "rewards" do
      ...
      it "should create a reward" do
        ...
        @browser.override_api_response_data :create_reward, data: factory(:reward)
      ...
      it "should update a reward" do
        @browser.li(text: "You will have a voucher").when_present.click
        @browser.text_field(name: 'description', value: 'You will have a voucher').when_present.set "Just kidding"
        @browser.override_api_response_data :update_reward, data: factory(:reward).merge(description: "Just kidding")
      ...
      it "should delete a reward" do
        ...
        @browser.override_api_response_data :destroy_reward, data: {}
        @browser.a(text: 'Delete').click

        Watir::Wait.until { !@browser.li(text: 'You will have a voucher').present? }
      end
      ...
      it "should insert many rewards with different amounts, sorting them as they are added" do
        ...
        @browser.override_api_response_data :create_reward, data: factory(:reward).merge(description: "Third", amount: 50)
        @create_form.button.click
        ...
        @browser.override_api_response_data :create_reward, data: factory(:reward).merge(description: "First", amount: 10)
        @create_form.button.click
        ...
        @browser.override_api_response_data :create_reward, data: factory(:reward).merge(description: "Fourth", amount: 100)
        @create_form.button.click
        ...
        @browser.override_api_response_data :create_reward, data: factory(:reward).merge(description: "Second", amount: 25)
        @create_form.button.click
    ...
    it "should now have a publish button" do
      @browser.div(text: 'Publish Fundraiser').wait_until_present
    end
